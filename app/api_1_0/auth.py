...
#
#
# @sysapi.before_app_request
# def before_request():
#     if current_user.is_authenticated:
#         current_user.ping()
#
#
# @sysapi.route('/authsys/api_1_0/account/login', methods=['POST'])
# def login():
#     """
#     1、获取请求参数用户名、密码
...
class Login(Resource):
    def post(self):
        ...
        passwd = getpassword(args['password'], fistkey, secondkey, thirdkey)
        ...
        # 用请求参数中的username查询数据库判断登录用户是否存在
        user = Users.query.filter_by(account=args['username']).first()
        # print user.password
        # 如果用户存在并且验证用户密码与数据库hash后的值相等
        # if user is not None and user.verify_password(passwd):
